  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        if (!valid)
          console.warn(
            '[BouncyDupesR3F] HDR not available or invalid. Using standard lights.',
          );
      } catch {
        if (!alive) return;
        setOk(false);
        console.warn(
          '[BouncyDupesR3F] HDR fetch failed. Using standard lights.',
        );
      }
    })();
    return () => {
...
function ShapeCollider({ shapeKey, scale }) {
  ...
  if (shapeKey === 'sphere')
    return <BallCollider args={[0.6 * s]} restitution={1.0} friction={0} />;
  if (shapeKey === 'cube')
    return (
      <CuboidCollider
        args={[0.6 * s, 0.6 * s, 0.6 * s]}
        restitution={1.0}
        friction={0}
      />
    );
...
export default function BouncyDupesR3F({
  ...
  const [items, setItems] = useState(() => [
    {
      ...
      scale: 1,
      pos: [
        randomInRange(-bounds * 0.6, bounds * 0.6),
        0.75,
        randomInRange(-bounds * 0.6, bounds * 0.6),
      ],
  ...
  const onDuplicate = useCallback(
    (id) => {
      ...
      setItems((prev) => {
        ...
        const jitter = 0.6;
        const nx = clamp(
          x + randomInRange(-jitter, jitter),
          -bounds * 0.7,
          bounds * 0.7,
        );
        const nz = clamp(
          z + randomInRange(-jitter, jitter),
          -bounds * 0.7,
          bounds * 0.7,
        );
  ...
  const disableControlsForThisPointerDown = useCallback((e) => {
    const ctrls = controlsRef.current;
    if (!ctrls) return;
        ...
        <div className='absolute left-3 top-14 z-10 w-[280px] rounded-xl bg-black/60 p-3 text-white shadow-lg backdrop-blur'>
          <div className='flex flex-col gap-3'>
              ...
              <input
                ...
                onChange={(e) => {
                  ...
                  requestAnimationFrame(() =>
                    snapshotPose(`TOGGLE ${next ? 'ON' : 'OFF'} RAF after`),
                  );
                }}
              />
            ...
            <div className='text-xs text-white/70'>
              Click duplicates + shrinks:{' '}
              <span className='text-white/90'>1 → 0.8 → 0.6 → 0.4 → 0.2</span>.
              <br />
              Collision is now more shape-appropriate (primitives + convex
              hull).
            </div>
          </div>
        </div>
